...
%% Load data.
path = "/home/abajcsy/hybrid_ws/src/hallucinate/data_for_paper/sim_example_bayes/";
...
predsColor = [linspace(0, teal(1), numPredsColors); ... 
    linspace(0, teal(2), numPredsColors); ... 
    linspace(0, teal(3), numPredsColors)].'; 
predsAlpha = linspace(0.3, 0.9, length(human_pos));
humanGoalColor = 'r';
...
%         [c, h] = contourf(g.xs{1}, g.xs{2}, p, [0,1]);
%         X = c(1,2:end);
%         Y = c(2,2:end);
%         delete(h);
...
%         end
        ...
        [c, h] = contour(g.xs{1}, g.xs{2}, p, [0,0.01]);
        h.EdgeColor = color(idx,:);
        h.LineWidth = 1;
        X = c(1,2:end);
        Y = c(2,2:end);
        if idx > 1
            badIndicies = find(Y>5);
            Y(badIndicies) = [];
            X(badIndicies) = [];
        end
        %delete(h);
