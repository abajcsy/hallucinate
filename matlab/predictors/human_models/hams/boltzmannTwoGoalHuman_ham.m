function hamValue = boltzmannTwoGoalHuman_ham(t, data, deriv, schemeData)
...
% % end
% 
% hamU = human.likelyCtrls{1};
% xdot = human.dynamics(1,x,hamU);
% hamValue = min(hamValue, hamU);
% 
% for i=2:human.numCtrls
%     % Get dynamics at each state given current control.
%     %xdot = human.xdot{i};
%     u = human.likelyCtrls{i};
%     xdot = human.dynamics(1,x,u);
%     
%     % Compute Hamiltonian: 
%     %   \grad_x V(x,t) * f(x,u).
